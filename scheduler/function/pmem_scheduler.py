...
class EventQueue:
    ...
    def schedule(self, delta_t, priority, callback):
        heapq.heappush(self.events, (self.t, priority,
                                     self.i, callback))  # Heap Quque
        self.i += 1
...
class ClosedLoopWorkload:
    ...
    def start_next_request(self):
        resource_requirements = self.request_generator.make_request(
            self.random)
...
class Request:
    ...
    def __init__(self, workload, admission_control, resource_requirements):
        ...
        self.admission_control = admission_control
        self.pending_stages = [ResourceStage(self, r, q, i) for (
            i, (r, q)) in enumerate(resource_requirements)]
    ...
    def execute_next_stage(self):
        ...
        #process = subprocess.Popen(['taskset', '-c', str(mapping[0]), 'sh', './memory_profiler', str(thr[0])])
        #cmd = "taskset -c {0} sh ./memory_profiler {1} &".format(mapping[r],thr[r])
        # os.system(cmd)
...
class VirtualTimeConsumptionTracker:
    ...
    def _schedule_next_completion(self):
        if len(self.ongoing) > 0:
            ...
            delta_t = (finish_vt - self.vt) * \
                (len(self.ongoing) / self.capacity)
            q.schedule(delta_t, Priority.RESOURCE_EXEC_COMPLETE, partial(
                self._check_completions_callback, self.iteration, finish_vt))
...
class Resource:
    ...
    def _exec_next(self):                             # Dequeueing a job
        while self.count < self.concurrency and not self.queue.is_empty():
            ...
            logEvent(self.name.upper(), "Dequeue %s: %s.  Queued for %s" % (
                next_execution, next_execution.verbose_description(), t_str(qtime)))
            self.consumption_tracker.add(next_execution.quantity, partial(
                self._on_execution_completed, next_execution))
    ...
    def enqueue(self, execution):                    # Enqueueing a job
        ...
        logEvent(self.name.upper(), "Enqueue   %s: %s" %
                 (execution, execution.verbose_description()))
    ...
    def _on_execution_completed(self, execution):    # Execution of a job
        ...
        logEvent(self.name.upper(), "Completed %s: %s.  Executed for %s" %
                 (execution, execution.verbose_description(), t_str(etime)))
...
function = Resource("function", capacity=1000000,
                    queue=FIFOQueue(), concurrency=100)
admissionqueue = FIFOQueue()
admissioncontrol = FixedConcurrencyAdmissionControl(1, admissionqueue)
workload1generator = RequestGenerator().binomial(
    function, 10, 100)  # Workload generator
workload1 = ClosedLoopWorkload(workload_id=1, request_generator=workload1generator,
                               concurrency=concurrency_generator(), admission_control=admissioncontrol)

total_time = 50  # Time-frame
