...
void timeitp(void (*only_write)(void*, size_t), char* name) {
  double min = INFINITY;
  size_t i;
  for (i = 0; i < SAMPLES; i++) {
...
#pragma omp master
      ...
      int j;
      for (j = 0; j < TIMES; j++) {
	only_write(&array[chunk_size * omp_get_thread_num()], chunk_size);
      }
#pragma omp barrier
#pragma omp master
...
void timeit(void (*only_write)(void*, size_t), char* name) {
  double min = INFINITY;
  size_t i;
  for (i = 0; i < SAMPLES; i++) {
    double before, after, total;
